...
void Captcha::updateCaptcha()
{
	...
	if (m_deformationType == Deform_SinCurve)
	{
		...
		qreal sinrandomness = ((qreal) randomGen.generateDouble() / RAND_MAX) * 5.0;
		...
		m_captchaImage = QImage(fm.horizontalAdvance(m_captchaText) + m_vmod2 * 2 + m_padding * 2,
	...
	if ( m_drawLines )
	{
		...
		{
			int x1 = randomGen.generateDouble() * m_captchaImage.width();
			int y1 = randomGen.generateDouble() * m_captchaImage.height();
			int x2 = randomGen.generateDouble() * m_captchaImage.width();
			int y2 = randomGen.generateDouble() * m_captchaImage.height();
	...
	if (m_drawEllipses)
	{
		...
		{
			int x1 = m_ellipseMaxRadius / 2.0 + randomGen.generateDouble() * ( m_captchaImage.width() - m_ellipseMaxRadius );
			int y1 = m_ellipseMaxRadius / 2.0 + randomGen.generateDouble() * ( m_captchaImage.height() - m_ellipseMaxRadius );
			int rad1 = m_ellipseMinRadius + randomGen.generateDouble() * ( m_ellipseMaxRadius - m_ellipseMinRadius );
			int rad2 = m_ellipseMinRadius + randomGen.generateDouble() * ( m_ellipseMaxRadius - m_ellipseMinRadius );
...
void Captcha::generateText(int noOfChars, bool includeNumbers, bool includeSymbols, bool allCapital)
{
	...
	if (m_textGeneration == TextGeneration_Random)
	{
		...
		{
			text+= static_cast< char >( randomGen.bounded( 65, 91 ) );
			/*if (!allCapital) chars.push_back(97 + ((qreal) randomGen.generateDouble() / RAND_MAX) * (122 - 97));
			if (includeNumbers) chars.push_back(48 + ((qreal) randomGen.generateDouble() / RAND_MAX) * (57 - 48));
			if (includeSymbols) chars.push_back(33 + ((qreal) randomGen.generateDouble() / RAND_MAX) * (47 - 33));*/
		}
	...
	else if (m_textGeneration == TextGeneration_Dictionary)
	{
		...
		m_captchaText = m_dictionary[( randomGen.generateDouble() / (qreal) RAND_MAX) * (m_dictionary.size() - 1.0)];
	}
	else
	{
